# Terraform plan for Kubernetes master servers on Proxmox.
# ---

# Pi-Hole DNS
resource "proxmox_virtual_environment_vm" "k8s-master" {
  for_each = var.k8s-master
  ...
  description    = var.common-k8s.desc-master
  ...
  cpu {
    ...
    cores        = var.common-k8s.cores-master
  }
  memory {
    dedicated    = var.common-k8s.memory-master
