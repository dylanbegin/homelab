...
source "proxmox-iso" "centos-10gb" {
  ...
  vm_id                = "1011"
  vm_name              = "centos-10gb"
  template_description = "Centos Base Template"

  # VM OS Settings
  iso_storage_pool = "nomad"
  iso_file         = "nomad:iso/centos-9.iso"
  unmount_iso      = true
  os               = "l26"
...
build {
  ...
  name    = "centos-10gb"
  sources = ["source.proxmox-iso.centos-10gb"]
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo dnf remove ethtool firewalld linux-firmware man-db sg3_utils-libs sssd-client -y",
