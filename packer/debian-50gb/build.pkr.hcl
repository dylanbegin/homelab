...
source "proxmox-iso" "debian-50gb" {
  ...
  boot_command = [
    ...
    " auto=true console=ttyS0 ipv6.disable=1 hostname=debian-50gb domain=localhost url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ps.cfg",
    "<wait><f10>"
  ]
...
build {
  ...
  name    = "debian-50gb"
  sources = ["source.proxmox-iso.debian-50gb"]
